...
Frame header: ASCII PM02 + little-endian uint32 sequence + float64 capture milliseconds. Receiver ACK every 100 ms; legacy raw PCM supported for 0.1 clients, without v2 freshness protections. Single-device takeover uses a client UUID and token; only the active socket owns buffer cleanup. Native TLS verifies the QR-pinned CA plus IP and chain validity.
